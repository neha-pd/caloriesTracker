...
export default function WatchSettings(){
 ...
 return <Page back title="Your watch" subtitle="FitLens reads the activity your watch shares with your phone.">
  <Card><T bold size={20}>{s.enabled?"Watch sync is on":"Connect your watch"}</T>
   <T color={C.muted}>{s.enabled?"Open FitLens after your watch syncs. Your readings refresh automatically.":`First sync your watch with its companion app. Then connect ${platform} to bring your activity into FitLens.`}</T>
   {s.lastSync&&<T size={12} color={C.muted}>Last synced {new Date(s.lastSync).toLocaleString()}</T>}
   {native?<Button title={s.enabled?"Sync now":"Connect watch"} loading={s.busy} onPress={()=>void(s.enabled?s.sync():s.connect())}/>:<T color={C.muted}>Use the FitLens phone app to connect your watch.</T>}
  </Card>
  ...
  {s.enabled&&<>
   <Section title="Bring in past activity"><T color={C.muted}>Choose how far back to look. Keep FitLens open while it imports the readings your watch has shared.</T><Segments value={period} onChange={setPeriod} values={[{key:"month",label:"30 days"},{key:"year",label:"This past year"}]}/><Button title={s.importing?`Importing · ${s.importProgress}%`:"Import activity"} disabled={s.importing||!native} onPress={()=>void s.importHistory(period==="year")}/></Section>
   <Section title="Connection help"><T color={C.muted}>Missing a reading? Sync your watch in its companion app, then check that FitLens has permission to read it.</T>
   ...
   {disconnect&&<Card><T>Stop syncing this watch? Your previously saved readings will stay in FitLens.</T><Button danger title="Yes, disconnect" onPress={()=>void s.disconnect()}/><Tap label="Keep connected" onPress={()=>setDisconnect(false)}><T color={C.lime}>Keep connected</T></Tap></Card>}
  </>}
 </Page>;
}
