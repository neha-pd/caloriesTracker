...
with sync_playwright() as p:
 ...
 page.get_by_role('button',name='Activity & burn details',exact=True).click();expect(page.get_by_role('button',name='Import activity',exact=True)).to_have_count(0);expect(page.get_by_label('Daily step goal · optional',exact=True)).to_have_count(0);page.get_by_role('button',name='Edit goals',exact=True).click();page.get_by_label('Daily step goal · optional',exact=True).fill('6000');page.get_by_label('Active kcal goal · optional',exact=True).fill('350');page.get_by_label('Workout minutes goal · optional',exact=True).fill('30');page.get_by_role('button',name='Save movement goals',exact=True).click();expect(page.get_by_text('Movement goals saved.',exact=False)).to_be_visible()
