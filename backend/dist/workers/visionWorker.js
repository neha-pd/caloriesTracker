...
async function processVisionJob(job) {
  ...
  `;
    ...
    let nutrients;
    try {
        nutrients = await getNutrientsForItems(geminiResult.items.map(item => ({
            name: item.name,
    ...
    UPDATE log_entries SET
      ...
      ai_identified_items  = ${JSON.stringify(geminiResult.items.map((item, i) => ({
        ...item,
...
export function startVisionWorker() {
    const worker = new Worker('vision-jobs', processVisionJob, {
        connection: redis,
...
//# sourceMappingURL=visionWorker.js.map