...
with sync_playwright() as p:
 ...
 page.get_by_role('button',name='Log activity',exact=True).click();page.get_by_label('Activity name',exact=True).fill('Evening walk');page.get_by_label('Duration · minutes',exact=True).fill('40');page.get_by_label('Active calories · optional',exact=True).fill('180');context.set_offline(True);page.get_by_role('button',name='Save record',exact=True).click();expect(page.get_by_test_id('daily-energy').get_by_text('180',exact=True)).to_be_visible();expect(page.get_by_test_id('daily-energy').get_by_text('Unavailable',exact=True)).to_be_visible()
