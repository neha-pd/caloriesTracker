...
export async function searchNutritionix(query, limit = 10) {
    ...
    return items.map((item) => ({
        ...
        calories: item.nf_calories ?? item.full_nutrients?.find((n) => n.attr_id === 208)?.value ?? 0,
...
export async function getNutrientsForItems(items) {
    ...
    const data = await res.json();
    const foods = (data.foods ?? []).map((f, i) => ({
...
export async function getNutritionixById(id) {
...
//# sourceMappingURL=nutritionix.js.map