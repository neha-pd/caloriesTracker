...
function calculateNutrition(user) {
    ...
    if (goal_type === 'lose_weight')
        tdee = Math.max(500, Math.round(tdee * 0.85));
...
export async function userRoutes(app) {
    ...
    app.get('/me/goals/calculate', { onRequest: [app.authenticate] }, async (request, reply) => {
        const userId = request.user.sub;
        const { goal_type: queryGoalType } = request.query;
        const [user] = await db `
    ...
    `;
        ...
        if (!result)
            return reply.status(422).send({ error: 'Please complete your physical profile (age, height, weight) first.' });
